...
export const TransferModal: React.FC<TransferModalProps> = ({
  ...
  useEffect(() => {
    if (isSuccess && !hasProcessedSuccessRef.current) {
      ...
      const newQuantity = initialQuantityRef.current - transferredAmountRef.current;
        ...
        <div className="space-y-4">
          {currentQuantity === 0 && (
            <div className="bg-yellow-500/20 border border-yellow-500/50 rounded-lg p-3">
              <p className="text-yellow-300 text-sm">
                This NFT has no available tokens for transfer. You can still view the details below.
              </p>
            </div>
          )}
            ...
            <div className="bg-gray-800 p-3 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-gray-400 text-sm break-all">Token ID: {nft.tokenId}</p>
                  <p className={`text-sm ${currentQuantity === 0 ? 'text-red-400' : 'text-gray-400'}`}>
          ...
          {error && (
            <div className="bg-red-500/20 border border-red-500/50 rounded-lg p-3">
              <p className="text-red-300 text-sm break-words">{error.message}</p>
            </div>
          )}
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              ...
              disabled={!isValidAddress || isPending || isConfirming || amount < 1 || currentQuantity === 0}
            ...
            >
              {currentQuantity === 0 ? "No Tokens Available" : isPending || isConfirming ? "Transferring..." : "Transfer"}
            </button>
          </div>
        </div>
      </div>
